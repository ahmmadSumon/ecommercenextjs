...
const Cart = () => {
  ...
  return (
    <section className="bg-white dark:bg-black  py-8 mt-20 antialiased  md:py-16">
        ...
        <div className="mt-6 sm:mt-8 md:gap-6 lg:flex lg:items-start xl:gap-8">
          <div className="lg:w-3/4 min-h-[50vh]">
            {items.length === 0 ? (
              ...
              items.map((item) => {
                console.log('Item ID:', item.id);  // Log the item id from the cart
                const product = allProducts.find(
                  (prod) => String(prod.id) === String(item.id)  // Convert both to string for comparison
                );
                if (!product) {
                  console.log('Product not found for ID:', item.id);  // Log if product isn't found
                  return <p key={item.id}>Product not found</p>;
                }
